...
function InfoRow({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-3 border-b border-border last:border-0">
      ...
      <span className="text-sm text-foreground text-right">{value || t.pages.patients.noData}</span>
    </div>
  );
}
  ...
  if (!patient) {
    return (
      <div className="text-center py-16">
        ...
        <Link href="/patients" className="mt-4 inline-block text-sm text-primary hover:underline">
          {p.backToList}
        </Link>
      </div>
    );
      ...
      <div className="rounded-2xl border border-border bg-surface p-5 space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-sm font-semibold text-foreground">{t.pages.history.title}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              {t.pages.history.viewAll}
            </Link>
            <Link
              href={`/patients/${id}/history/new`}
        ...
        {recordsLoading ? (
          ...
          <div className="py-4 text-center">
            <p className="text-sm text-muted">{t.pages.history.empty}</p>
            <p className="mt-0.5 text-xs text-muted/70">{t.pages.history.emptySubtitle}</p>
          </div>
        ) : (
          <div className="space-y-2">
            {recentRecords.map((record) => (
              <Link
                key={record.id}
                href={`/patients/${id}/history/${record.id}`}
